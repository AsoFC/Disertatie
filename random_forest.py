...
def cv(name, ens, df, y, splits=10):
    ...
    for train_index, test_index in kf.split(df):
        ...
        # print("bye")
        break
    print(name, accuracy_score(y_true, y_pred), time() - t)
...
if __name__ == "__main__":
    ...
    ens = ExtraDiffObliqueForrest('classification', 11, 5, 2)
    
    
    t = time()
    # et = ExtraTreeForrest('classification', 21, 20, 10)
    ens = ExtraTreeForrest('classification', 11, 5, 2)
    cv('etf', ens, df, y)    
    
    ens = ExtraTreesClassifier(n_estimators=11, min_samples_leaf=5, 
                                  max_features=2, random_state=seed)
    cv('sketf', ens, x, y)
    
    ens = RandomForestClassifier(n_estimators=11, min_samples_leaf=5, 
                                  max_features=2, random_state=seed)
